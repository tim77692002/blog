...
public class CalendarController {
	...
	@RequestMapping(value = "/admin/cal/delete", method = RequestMethod.POST)
	...
	public ModelAndView calendar_home(HttpServletRequest request) throws Exception {
		...
		while (cal.get(Calendar.MONTH) == month) {
			if (cal.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY) {
				sb.append("<td class='calendar_td' valign='top'><span style='color:blue;'>" + cal.get(Calendar.DATE)
						+ "</span>"); // 토요일인경우
			} else if (cal.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY) {
				sb.append("<td class='calendar_td' valign='top'><span style='color:red;'>" + cal.get(Calendar.DATE)
						+ "</span>"); // 일요일인 경우
			} else {
				sb.append("<td class='calendar_td' valign='top'>" + cal.get(Calendar.DATE)); // 월
																								// ~
																								// 금요일
			}

			// str = cal.get(Calendar.YEAR) + "-" + cal.get(Calendar.MONTH) +
			...
			if (list != null) { // 날짜당 일정 추출
				for (int i = 0; i < list.size(); i++) {
					...
					sb.append("<a href='./cal/update?calendarno=" + dto.getCalendarno() + "'>" + dto.getLabel()
							+ "</a><br>");
				}
			...
			if ((cal.getActualMaximum(Calendar.DAY_OF_MONTH) != cal.get(Calendar.DATE))) {
				if (cal.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY) { // 토요일인
																			// 경우
					sb.append("</tr><tr>");
				}
			}
		...
		while (cal.get(Calendar.MONTH) == month) {
			...
			if (list != null) { // 날짜당 일정 추출
				for (int i = 0; i < list.size(); i++) {
					...
					sb.append("<img src='" + Utility.getRoot() + "/images/bu5.gif'>");
					sb.append(
							"<a href='./update?calendarno=" + dto.getCalendarno() + "'>" + dto.getLabel() + "</a><br>");
				}
